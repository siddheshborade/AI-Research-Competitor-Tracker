...
export function IntelligenceDashboard() {
  ...
  const hasActiveInvestigation = Boolean(activeObjective && agentStatus);
  ...
  return (
    <div className="space-y-7 max-w-7xl mx-auto pb-16 font-sans">
      ...
      <div className="bg-[#0D0F16] rounded-2xl p-6 sm:p-7 border border-[#1A1F2C] shadow-nexus-card flex flex-col lg:flex-row lg:items-center justify-between gap-6 relative overflow-hidden">
        <div className="space-y-2.5 relative z-10 max-w-2xl">
          ...
          <p className="text-xs sm:text-sm text-slate-400">
            Monitor research, competitors, patents, news, and emerging threats with autonomous AI investigations.
          </p>
        </div>
      ...
      {hasActiveInvestigation ? (
        ...
        <div className="bg-[#0D0F16] rounded-2xl p-6 border border-[#1A1F2C] text-center space-y-3">
          <Inbox className="w-8 h-8 text-slate-600 mx-auto" />
          <div className="space-y-1">
            <h3 className="text-sm font-bold text-slate-200">No active investigation</h3>
            <p className="text-xs text-slate-400">Start a new research investigation to begin autonomous surveillance.</p>
          </div>
          <button
            onClick={() => setActiveView("landing")}
            className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-[#7C2CFF] text-white text-xs font-semibold"
          >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-[#0D0F16] rounded-2xl p-5 border border-[#1A1F2C] space-y-2">
          <div className="flex items-center justify-between text-slate-400">
            <span className="text-xs font-mono font-medium">Active Investigations</span>
            <Activity className="w-4 h-4 text-[#00D9FF]" />
          </div>
          <div className="text-2xl font-bold font-mono text-slate-100">
            {hasActiveInvestigation ? 1 : 0}
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 bg-[#0D0F16] rounded-2xl p-6 border border-[#1A1F2C] space-y-4">
          ...
          {agentSteps.length > 0 ? (
            ...
            <div className="p-6 text-center text-xs text-slate-400 bg-[#07080D] rounded-xl border border-[#1A1F2C]">
              No agent activity yet. Dispatches will appear here during execution.
            </div>
          )}
        </div>
      </div>
      ...
      <div className="bg-[#0D0F16] rounded-2xl p-6 border border-[#1A1F2C] space-y-4">
        ...
        {(memoryHistory?.investigations || []).length > 0 ? (
            ...
            {(memoryHistory?.investigations || []).slice(0, 2).map((inv, idx) => (
              ...
              >
                ...
                <div className="flex items-center justify-between pt-1 text-[10px] font-mono">
                  <span className="text-emerald-400 font-bold">Evidence Confidence: {Math.round((inv.confidence_score || 0.88) * 100)}%</span>
